...
if (!message.member.roles.has('696338093296320562') && !message.member.hasPermission('ADMINISTRATOR'))
  ...
  if(!member.roles.has(`696338104688181298`)) {
    member.addRole(`696338104688181298`)
    let kayıt = new Discord.RichEmbed()
    .setColor("BLACK")
   .addField(`Andrømeda ❅`, `${kişi} **adlı üyeye** <@&696338104688181298> **rolü verildi!**`)
   .setFooter(`Komutu kullanan yetkili : ${message.author.username}`) 
    return message.channel.send(kayıt).then(msg => msg.delete(5000)).then(() => message.react(emoji));
  } else {
    member.removeRole(`696338104688181298`)
    let kayıt = new Discord.RichEmbed()
    .setColor("BLACK")
    .addField(`Andrømeda ❅`, `${kişi} **adlı üyeden** <@&696338104688181298> **rolü alındı!**`)
    .setFooter(`Komutu kullanan yetkili : ${message.author.username}`) 
    return message.channel.send(kayıt).then(msg => msg.delete(5000)).then(() => message.react(emoji));
  }
};
